...
### Task 5: Fix partial defrag logging, restore Java 8 runtime, and merge

Discovered during Session 1 code review: if `Defragment.defrag(pool)` fails, `dbusers` silently skips defrag and the completion log still says "complete." Also, installing `temurin-11-jdk` changed the system default java to 11 — Jersey 2.14 throws HK2 warnings at runtime on Java 11. Both must be fixed before merging.

**Files:**
- Modify: `src/main/java/strat/mining/stratum/proxy/database/DatabaseManager.java`

- [ ] **Step 1: Split try/catch per-database in `defragment()`**

Replace the single try block covering both defrag calls with two separate try/catch blocks so each database failure is independently logged:
...
```bash
git add src/main/java/strat/mining/stratum/proxy/database/DatabaseManager.java
git commit -m "fix: split defragment() try/catch per-database for clearer failure logging"
```

- [ ] **Step 4: Build and deploy updated jar**
...
- [ ] **Step 5: Restore Java 8 as system default (run manually with sudo)**

Installing `temurin-11-jdk` switched the system default java to 11. Jersey 2.14 has HK2 runtime warnings on Java 11. The proxies must run on Java 8.

```bash
sudo update-alternatives --set java /usr/lib/jvm/temurin-8-jre-amd64/bin/java
java -version  # must show 1.8
```

- [ ] **Step 6: Restart all proxy services (run manually with sudo)**
...
Verify clean startup (no HK2/Jersey warnings):

```bash
grep -i "error\|NoClassDef\|reification" /home/mine/proxs/stratum-proxy-gr-n/stratum-proxy.log | tail -5
```

Expected: no output.

- [ ] **Step 7: Merge to master using `superpowers:finishing-a-development-branch`**

Invoke the `superpowers:finishing-a-development-branch` skill — it guides the merge/PR/cleanup decision for the `feature/db4o-defrag` branch.
